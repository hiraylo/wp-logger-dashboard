...
class HomeController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($daterange) {
            ...
            if ($website) {
                $log = Log::whereIn( 'website_name', $website )->whereBetween(DB::raw('DATE(created_at)'), array($daterange[0], $daterange[1]))->orderBy('created_at', 'DESC')->skip(0)->take($record_count)->get();
            } else {
            ...
            $log = Log::whereIn( 'website_name', $website )->orderBy('created_at', 'DESC')->skip(0)->take($record_count)->get();
    ...
    {
        $websites = DB::table('logs')->orderBy('website_name', 'ASC')->pluck('website_name')->toArray();
